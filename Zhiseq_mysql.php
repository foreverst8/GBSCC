...
if(is_uploaded_file($_FILES['sample_file']['tmp_name'])){
	...
	if($err_count>0){
		...
		if (!$res){
			...
			$main_mesg="Dear Core user,<br><br>".$_SESSION['username']." from ".$result_user[0]['lab']." has submitted samples for NGS sequencing. The Sample ID is <a href=\"http://161.64.198.12/GBSCC/hiseq_search_result_detial.php?ID=$Hiseq_Sample_ID\">$Hiseq_Sample_ID</a>. You can find the sample information below and on the <a href=\"http://161.64.198.12/GBSCC/index.php\">Core Database</a>.<br><br>This is an automated email from the <a href=\"http://161.64.198.12/GBSCC/index.php\">Core database</a>. Please do not reply to this email address. For any queries, please contact the Core Support team.";
