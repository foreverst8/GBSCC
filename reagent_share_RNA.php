  ...
  <tr>
   ...
   <?php require('login.php');?> 
	...
	if(!$_GET['Submitter_Name']){
    	...
    	echo "<form action=\"reagent_share_RNA.php\" method=\"get\">";
		$_SESSION['resubmit']=1;
		...
		echo "<b>Request RNA Sample Count: </b>";
	
		echo "<input name=\"request_count\" id=\"request_count\" type=\"number\" min=\"0\" max=\"10\" style=\"width:100px\">";
		echo "</td>";
		
		echo "<td align=\"center\" >How many days for keeping the request: ";
		echo "<input name=\"request_day\" id=\"request_day\" type=\"number\" value=\"7\" min=\"0\" max=\"30\" style=\"width:80px\">";
		echo "</td>";
		...
		$result_share_reagent=search("select * from share_reagent_request_RNA where state='Open'");
		$conn = db_connect();
		...
		for($i=0;$i<count($result_share_reagent);$i++){
			...
			if($Date_today-$date_tmp>$result_share_reagent[$i]['Date']){
					$res_tmp=$conn->query("UPDATE genomics_core.share_reagent_request_RNA SET state='Closed' where share_reagent_request_id='".$result_share_reagent[$i]['share_reagent_request_id']."'");
		...
		if(count($result_share_reagent)>0){
			...
			for($i=0;$i<count($result_share_reagent);$i++){
				...
				if($result_share_reagent[$i]['Submitter_Name']==$_SESSION['username'] or $result_user[0]['main']=='y'){
					echo "<td><a href=\"delete_shared_request.php?id=".$result_share_reagent[$i]['SRR_ID']."&database=share_reagent_request_RNA\"><input type=\"button\" value=\"DELETE\"></a></td>";
				}
				else{
					echo "<td>&nbsp;</td>";	
				}
	 ...
	 if($_SESSION['resubmit']==1){
		...
		$result_tmp=search("select max(share_reagent_request_id) from genomics_core.share_reagent_request_RNA");
		...
		for($j=0;$j<$request_count;$j++){
			...
			$conn = db_connect();
			$res=$conn->query("INSERT INTO genomics_core.share_reagent_request_RNA($name) VALUES (".$value.")");

			if (!$res){
				echo "Error: Request Faild.<br>INSERT INTO genomics_core.share_reagent_request_RNA($name) VALUES ($value)";
			}
		...
		$result_share_reagent=search("select * from share_reagent_request_RNA where state='Open'");
		if(count($result_share_reagent)<12){
			...
			echo "<span style=\"color:red\">";
			echo "<br><br>1. Your have <b style=\"font-size:18px\">".$request_user_count[$result_user[0]['user_name']]."</b> samples pass the RNA share request successfully.<br><br>";
			echo "</span>";
			...
			$value="'".($result_reagent_tmp[0]['max(reagent_id)']+1)."','$RgRID','1','350','$select_user','$select_user_email','$select_user_lab','$Date','$Remark_comb'";
			$name="reagent_id,RgRID,RNA_Nano,total_cost,Submitter_Name,email,lab,date,remark";
			...
			$request_record.="RNA Nano Chip (12 Samples/Chip), Quantity(Per Chip) :  1<br><br>$Remark_comb";	
			...
			if (!$res){
				...
				for($ii=0;$ii<12;$ii++){
					$res_tmp=$conn->query("UPDATE genomics_core.share_reagent_request_RNA SET state='finished' where share_reagent_request_id='".$result_share_reagent[$ii]['share_reagent_request_id']."'");
					#echo "UPDATE genomics_core.share_reagent_request_RNA SET state='finished' where share_reagent_request_id='".$result_share_reagent[$ii]['share_reagent_request_id']."'<br>";
					if (!$res_tmp){
			...
			if(count($result_share_reagent)>12){
				...
				for($i=12;$i<count($result_share_reagent);$i++){
					echo "<tr valign=\"middle\" align=\"center\">";
					echo "<td>".($i+1-12)."</td>";
					echo "<td>".$result_share_reagent[$i]['SRR_ID']."</td>";	
					echo "<td>".$result_share_reagent[$i]['request_day']."</td>";	
		...
		$_SESSION['resubmit']++;
		echo "<br><br>You already submit your request. You can check from <a href=\"http://161.64.198.12/genomics_core/reagent_share_RNA.php\">RNA shared request</a> webpage<br><br>";	
	}
