...
<body>
    ...
    $overlap = search("select * from genomics_core.freezedrier");
    ...
    $result_tmp = search("select max(id) from genomics_core.freezedrier");
    ...
    $end3 = $end1 . " " . $end2;
    $full = $start3 . " - " . $end3;
    $title = $full . " " . $user . " (" . $lab . ")" . " " . strtoupper($chamber);

    $value1 = "'" . ($result_tmp[0]['max(id)'] + 1) . "','$user','$lab','$email','$date','$chamber','$startdate','$starttime','$enddate','$endtime','$first','$last'";
    $name1 = "id,user,lab,email,date,chamber,startdate,starttime,enddate,endtime,first,last";
    ...
    $res1 = $conn->query("INSERT INTO genomics_core.freezedrier ($name1) VALUES (" . $value1 . ")");
    $res2 = $conn->query("INSERT INTO genomics_core.freezedrier_booking ($name2) VALUES (" . $value2 . ")");
